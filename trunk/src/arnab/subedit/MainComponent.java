...
class Utils {
	...
	static public String toTimeString(long time) {
		...
		long hour=time;
		return padZeros(hour,2)+":"+padZeros(minutes,2)+":"
			+padZeros(seconds,2)+","+padZeros(millis,3);
	}
		...
		new DropTarget(this,new DropTargetListener() {
			public void dragEnter(DropTargetDragEvent dtde) {
				if (!dtde.isDataFlavorSupported(DataFlavor.javaFileListFlavor))
					dtde.rejectDrag();
				else dtde.acceptDrag(DnDConstants.ACTION_COPY);
			}
				...
				new PropertyChangeListener() {
					public void propertyChange(PropertyChangeEvent evt) {
						...
						for (int i=0; i<adjustors.size(); ++i) {
							if (adjustors.get(i).getTextBox().isFocusOwner()) {
								int newRow=adjustors.get(i).getLineNum();
